...
@interface OrderContentViewController ()
...
-(void)setRightbarbutton{
    UIBarButtonItem *item=[[UIBarButtonItem alloc]initWithTitle:@"收藏" style:UIBarButtonItemStylePlain target:self action:@selector(collect)];
    [item setTintColor:[UIColor blackColor]];
...
-(void)collect{
    ...
    //判断是否已收藏过----查询符合条件的个数---字符串查不了
   int result=[db intForQuery:[NSString stringWithFormat:@"select count(*) from t_collect where idid=%@",self.idid]];
        if (result==0) {
            BOOL isCollect=[db executeUpdate:@"insert into t_collect(idid,title,time,author,content) values(?,?,?,?,?)",self.idid,self.receiveTitle,self.receiveTime,self.receiveAuthor,self.receiveContent];
            if (isCollect) {
